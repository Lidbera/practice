...
const serverRender = async (req, res, next) => {
  // 이 함수는 404가 더야 하는 상황에 404를 띄우지 않고 서버 사이드 렌더링을 해 줌
  ...
  // https://reddux.js.org/recipes/server-rendering#security-considerations
  const stateString = JSON.stringify(store.getState()).replace(/</g, "\\u003c");
  const stateScript = `<script>__PRELOADED_STATE__=${stateString}</script>`; // 리덕스 초기 상태를 스크립트로 주입

  // 미리 불러와야 하는 스타일/스크립트를 추출
  const tags = {
    scripts: stateScript + extractor.getScriptTags(), // 스크립트 파부분에 리덕스 상태 넣기
